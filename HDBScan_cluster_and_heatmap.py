...
def get_cluster_map(locations, bounding_box, zoom):
    ...
    fig = px.scatter_mapbox(cluster.loc[cluster['cluster'] > -1], lat="Latitude", lon="Longitude", size='Count',
                            color='Count', color_continuous_scale=px.colors.sequential.Aggrnyl, zoom=zoom)
...
out_map = get_cluster_map(loc_df, florida_box, 6)
out_map.show()
plotly.offline.plot(out_map, filename='florida_heatmap.html')

out_map = get_cluster_map(loc_df, miami_box, 9)
out_map.show()
